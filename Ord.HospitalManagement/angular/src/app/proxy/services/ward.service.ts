...
export class WardService {
  ...
  getList = (input: CustomePagedAndSortedResultRequestWardDto, config?: Partial<Rest.Config>) =>
    this.restService.request<any, PagedResultDto<WardDto>>({
      method: 'GET',
      url: '/api/app/ward',
      params: { provinceCode: input.provinceCode, districtCode: input.districtCode, filterName: input.filterName, sorting: input.sorting, skipCount: input.skipCount, maxResultCount: input.maxResultCount },
    },
    { apiName: this.apiName,...config });
